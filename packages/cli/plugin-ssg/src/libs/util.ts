import { path, ROUTE_SPEC_FILE, fs, isSingleEntry } from '@modern-js/utils';
...
export const standardOptions = (ssgOptions: SSG, entrypoints: EntryPoint[]) => {
  ...
  if (ssgOptions === true) {
    ...
    if (
      isSingle &&
      typeof (ssgOptions as MultiEntryOptions).main === 'undefined'
    ) {
      return { main: ssgOptions as SingleEntryOptions };
    } else {
